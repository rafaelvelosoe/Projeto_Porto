...
');
	...
	public function select_ferro()
	{
		$res_lastId = array();
		$cmd = $this->pdo->query('SELECT * FROM tb_movimentacao 
		left join tb_modal ON tb_modal.cd_modal = tb_movimentacao.cd_modal where tb_modal.cd_modal = 2;
');
		$res_lastId = $cmd->fetchAll(PDO::FETCH_ASSOC);
		return $res_lastId;
	}
	public function select_navio()
	{
		$res_lastId = array();
		$cmd = $this->pdo->query('SELECT * FROM tb_movimentacao 
		left join tb_modal ON tb_modal.cd_modal = tb_movimentacao.cd_modal where tb_modal.cd_modal = 4;
');
		$res_lastId = $cmd->fetchAll(PDO::FETCH_ASSOC);
		return $res_lastId;
	}
	
	public function cadastrarMovimentacao($cd_terminal,$nm_terminal,$cd_entradaEsqDir,$nm_apelidoTerminal,$qt_capacidadeRodoviariaRec,$qt_capacidadeFerroviariaRec,$qt_capacidadeNavioRec,$cd_numeroTerminal,$qt_capacidadeRodoviariaExp, $qt_capacidadeFerroviariaExp,$qt_capacidadeNavioExp, $vl_armazem_nominal, $cd_categoria){
		$cmd=$this->pdo->prepare('SELECT cd_terminal FROM tb_terminal WHERE cd_terminal = :c');
